...
Якщо після виконання ви поврнете ```true``` то цей пакет даних не буде відправлено до зігбі сокету.
...
<br>Приклад: ```def conn_cb(ip, id) SLZB.log("ip: " .. ip .. " id: " .. id) return true end ZB.on_connect(conn_cb)``` - виведе в консоль IP та id нового клієнту та відхилить його підключення
